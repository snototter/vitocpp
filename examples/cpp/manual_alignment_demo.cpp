...
//  VCP_LOG_FAILURE("DISTORTION STUFF" << capture->DistortionAt(0) << " depth: " << capture->DistortionAt(1));
  ...
  //TODO clean up or remove this demo
...
//  k4a_transformation_t trafo = k4a_transformation_create(&calib);
  ...
  Kd.at<double>(0, 0) *= sx_depth;
  Kd.at<double>(0, 2) *= sx_depth;
  Kd.at<double>(1, 1) *= sy_depth;
  Kd.at<double>(1, 2) *= sy_depth;

  auto align = vcp::best::rgbd::CreateRgbdAlignment(Kc, Kd, R, t, cv::Size(warp_dest_width, warp_dest_height),
                                       cv::Size(depth_width, depth_height), Dc, Dd);
  ...
  while (capture->AreAllDevicesAvailable() && elapsed_ms < MAX_STREAMING_TIME_PER_CONFIG)
  {
    ...
    cv::Mat warped = align->AlignDepth2Color(depth_resized);//WarpDepth(depth_resized, trafo, warp_dest_width, warp_dest_height);
