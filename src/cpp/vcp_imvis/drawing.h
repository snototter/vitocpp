...
void DrawXYZAxes(cv::Mat &image, const cv::Mat &K, const cv::Mat &R, const cv::Mat &t, const cv::Vec3d &origin=cv::Vec3d(0.0, 0.0, 0.0),
                 double scale_axes=1.0, double scale_image_points=1.0, int line_width=2, int dash_length=-1,
                 double tip_length=0.1, const cv::Scalar &color_x=kAxisColorsBGR[0], const cv::Scalar &color_y=kAxisColorsBGR[1],
                 const cv::Scalar &color_z=kAxisColorsBGR[2]);
